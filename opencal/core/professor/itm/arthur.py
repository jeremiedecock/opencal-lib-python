"""Professor Arthur randomly pick cards in an "active set" that is progressively widened.
...
class ProfessorArthur:
    ...
    def update_card(self):
        if len(self._active_card_set) > 0:
            scores = [card['arthur_score'] for card in self._active_card_set]
    ...
    def current_card_reply(self, answer, hide=False, duration=None, confidence=None):
        ...
        if self._current_card is not None:
            self._current_card['arthur_reviews'].append(result)
            self._current_card['arthur_score'] = sum(self._current_card['arthur_reviews'][:-REVIEWS_HORIZON]) #/ min(len(self._current_card['arthur_reviews']), REVIEWS_HORIZON)
    ...
    def update_card_list(self, card_list):
        ...
        for card in self._full_card_set:
            self._current_card['arthur_reviews'] = []
            self._current_card['arthur_score'] = 0
