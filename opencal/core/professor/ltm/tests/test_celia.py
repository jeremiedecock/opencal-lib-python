...
def test_card_without_review():
    assert celia.assess(CARD_WITHOUT_REVIEW_1, DateMock) == 0
    assert celia.assess(CARD_BASIC_LEVEL_MINUS1_1, DateMock) == 0
    assert celia.assess(CARD_BASIC_LEVEL_MINUS1_2, DateMock) == 0

def test_card_made_today():
    assert celia.assess(CARD_MADE_TODAY, DateMock) == celia.GRADE_DONT_REVIEW_THIS_CARD_TODAY

def test_card_made_today_with_time():
    assert celia.assess(CARD_MADE_TODAY_WITH_TIME, DateMock) == celia.GRADE_DONT_REVIEW_THIS_CARD_TODAY

def test_card_made_yesterday_with_time():
    assert celia.assess(CARD_MADE_YESTERDAY_WITH_TIME, DateMock) == 0

def test_basic_cards():
    assert celia.assess(CARD_BASIC_LEVEL0_1, DateMock) == 0
    assert celia.assess(CARD_BASIC_LEVEL0_2, DateMock) == 0
    assert celia.assess(CARD_BASIC_LEVEL1_1, DateMock) == 1
    assert celia.assess(CARD_BASIC_LEVEL2_1, DateMock) == 2
    assert celia.assess(CARD_BASIC_LEVEL2_2, DateMock) == 2
    assert celia.assess(CARD_BASIC_LEVEL2_3, DateMock) == 2

def test_reviews_not_sorted():
    #assert celia.assess(CARD_REVIEWS_NOT_SORTED, BOGUS_CURRENT_DATE) == 2
    with pytest.raises(AssertionError):
        celia.assess(CARD_REVIEWS_NOT_SORTED, DateMock)

def test_ignore_premature_right_reviews():
    assert celia.assess(CARD_IGNORE_PREMATURE_RIGHT_REVIEWS, DateMock) == 1

def test_ignore_premature_bad_reviews():
    assert celia.assess(CARD_IGNORE_PREMATURE_BAD_REVIEWS, DateMock) == 0
...
def test_card_wrong_review_today():
    assert celia.assess(CARD_WRONG_REVIEW_TODAY, DateMock) == celia.GRADE_DONT_REVIEW_THIS_CARD_TODAY
    assert celia.assess(CARD_NOT_TO_BE_REVIEWED_TODAY, DateMock) == celia.GRADE_DONT_REVIEW_THIS_CARD_TODAY
...
def test_estimate_card_difficulty():
    ...
    assert celia.estimate_card_difficulty(CARD_DIFFICULTY_NONE, tag_difficulty_dict) == POINTS_NEUTRAL
    assert celia.estimate_card_difficulty(CARD_DIFFICULTY_EASY, tag_difficulty_dict) == POINTS_EASY
    assert celia.estimate_card_difficulty(CARD_DIFFICULTY_NEUTRAL, tag_difficulty_dict) == POINTS_NEUTRAL
    assert celia.estimate_card_difficulty(CARD_DIFFICULTY_DIFFICULT, tag_difficulty_dict) == POINTS_DIFFICULT
    assert celia.estimate_card_difficulty(CARD_DIFFICULTY_EASY_DIFFICULT, tag_difficulty_dict) == POINTS_DIFFICULT
    assert celia.estimate_card_difficulty(CARD_DIFFICULTY_DIFFICULT_EASY, tag_difficulty_dict) == POINTS_DIFFICULT
    assert celia.estimate_card_difficulty(CARD_DIFFICULTY_DIFFICULT_NEUTRAL, tag_difficulty_dict) == POINTS_DIFFICULT
...
def test_empty_card_list():
    ...
    """
    prof = celia.ProfessorCelia(EMPTY_CARD_LIST)
...
def test_one_card_skip():
    ...
    prof = celia.ProfessorCelia([card_1], date_mock=DateMock)
...
def test_right_wrong_and_hide_reply():
    ...
    prof = celia.ProfessorCelia([card_level0_age5, card_level0_age4],
                                      date_mock=DateMock)
...
def test_one_hidden_card():
    ...
    - `professor.current_card` returns `None` when card lists contains hidden
      cards only.
    """
    prof = celia.ProfessorCelia(copy.deepcopy(ONE_HIDDEN_CARD))
    assert prof.current_card == None
...
def test_three_cards_two_hidden():
    ...
    prof = celia.ProfessorCelia([card_1, card_2, card_3],
                                      date_mock=DateMock)
...
def test_init_right_answer_current_grade():  # TODO: change rdates in CARD_BASIC_LEVEL0_X to be consistent with Celia's policy with grade 0
    ...
    assert celia.assess(card_reviewed_today_right_age6, date_mock=DateMock, ignore_today_answers=True) == 0
    assert celia.assess(card_reviewed_today_right_age1, date_mock=DateMock, ignore_today_answers=True) == 0
